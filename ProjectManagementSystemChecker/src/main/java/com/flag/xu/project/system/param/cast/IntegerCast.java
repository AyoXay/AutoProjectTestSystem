 ...
 */
public class IntegerCast extends ParamCast {
    @Override
    public Object paramCast(Field field, Element foo, Method method) throws AnnotationConflictException {
        ...
        if (fieldType.equals(StandardDataType.INT.getType()) || fieldType.equals(StandardDataType.INTEGER.getType())) {
            Integer param = Integer.valueOf(foo.elementText(getParamName(field, method)) == null ? "0" : foo.elementText(getParamName(field, method)));
            return param;
        } else {
            return new DoubleCast().paramCast(field, foo, method);
        }
    }
}
